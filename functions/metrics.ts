...
export const handler: Handler = async (event) => {
    ...
    const [priceRaw, mcapRaw, feesRaw, donationsRaw] = await Promise.all([
        redis.zrange<string[]>(redisKeys.tsPrice, from, to, { byScore: true }),
        redis.zrange<string[]>(redisKeys.tsMcap, from, to, { byScore: true }),
        redis.zrange<string[]>(redisKeys.tsFeesCum, from, to, { byScore: true }),
        redis.zrange<string[]>(redisKeys.tsDonationsCum, from, to, { byScore: true })
    ]);
